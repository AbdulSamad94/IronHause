    ...
    try:
        resend.Emails.send({
            ...
            "html": _base_email(
                ...
                preview=f"{name} is interested in IronHause — {message[:80]}",
                badge_text="New Lead",
                badge_color=_CYAN,
                headline=f"New lead from {name}",
                subline="Your AI agent captured a new prospect. Their details are below.",
                accent=_CYAN,
                body_html=_lead_body(name, email, message, lead_id),
            ),
        })
...
async def notify_new_booking(booking_id: int, preferred_date: str, notes: str) -> None:
    ...
    try:
        resend.Emails.send({
            "from": "IronHause AI <onboarding@resend.dev>",
            "to": settings.NOTIFICATION_EMAIL,
            "subject": f"New Booking Request — {preferred_date}",
            "html": _base_email(
                title=f"New Booking: {preferred_date}",
                preview=f"Intro session requested for {preferred_date}. Booking #{booking_id}.",
                ...
                accent=_PURPLE,
                body_html=_booking_body(booking_id, preferred_date, notes),
            ),
        })
